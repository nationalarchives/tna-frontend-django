...
def cookieBanner(request, *args, **kwargs):
    template = loader.get_template("components/cookie-banner.html")
    params = request.GET.get("params")
    context = json.loads(params) if params else {}
    return HttpResponse(template.render(context, request))
...
def skipLink(request, *args, **kwargs):
    template = loader.get_template("components/skip-link.html")
    params = request.GET.get("params")
    context = json.loads(params) if params else {}
    return HttpResponse(template.render(context, request))
